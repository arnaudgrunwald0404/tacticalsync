...
import { Settings, CheckSquare2, AlignJustify, Layers, Bot, Trash2, Copy, Zap, X, Pin, Menu } from 'lucide-react';
...
export default function InboxPage() {
  ...
  const { tags, loading: tagsLoading, createTag, createWorkstream, renameTag, getOrCreate, reload: reloadTags } = useInboxTags(userId);
  ...
  const { items, loading: itemsLoading, addItem, updateItem, markDone, archive, deleteItem, addTagToItem, removeTagFromItem, cycleWorkflowStatus, syncBriefItem, pinItem, reload: reloadItems } = useInboxItems(userId, filter);
  ...
  const { items: allItems } = useInboxItems(userId, allFilter);
  ...
  const handleQuickCreateTag = useCallback(async (name: string) => {
    const color = TAG_COLORS[tags.length % TAG_COLORS.length];
    return createTag(name, 'project', color);
  ...
  return (
    <div className="flex h-full overflow-hidden bg-gray-100/80 gap-2 p-2 relative">
      ...
      {/* Main stream + drawer */}
      <div className="flex-1 flex min-w-0 overflow-hidden gap-2">
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden bg-white rounded-xl shadow-sm border border-gray-200/80">
        ...
        <div className={cn('flex-1 min-h-0 overflow-y-auto', isMobile && 'pb-36')}>
          {itemsLoading ? (
