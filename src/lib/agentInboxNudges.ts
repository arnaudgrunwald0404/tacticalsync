...
//
// Mirrored at supabase/functions/_shared/agentInboxNudges.ts so the agent-tick
// edge function (Deno) and this Vitest suite exercise the same rules — the
// edge function can't import from src/, so the two copies are hand-kept in
// sync, matching the existing convention for
// src/lib/calendar/matchEventToMember.ts /
...
export interface InboxNudgeAgentConfig {
   ...
   *  (see optInStatus below) — nudges never fire until a user has explicitly
   *  turned this on, either via Settings or the one-time opt-in prompt. */
 ...
 * instead of date-fns, since the mirrored edge function copy of this file
 * can't import from src/. `remainingDays <= 0` there corresponds to `true`
 * here: a due date whose calendar day has already arrived or passed. A null
 * due date is *not* the "now" tier (mirrors currentPriorityTier returning
 * null, not 'now', for a null dueAt).
 *
