...
describe('matchEventToMember', () => {
  ...
  it('passes the max-attendees filter with self + 1 other (default max=1)', () => {
    const member = makeMember({ id: 'm1', email: 'sam@x.com' });
    const event = makeEvent({
      attendees: [SELF, { email: 'sam@x.com' }],
    });
  ...
  it('filters out events with self + 2 others when max=1', () => {
    const member = makeMember({ id: 'm1', email: 'sam@x.com' });
    const event = makeEvent({
  ...
  it('allows larger meetings when max_other_attendees is raised', () => {
    const member = makeMember({ id: 'm1', email: 'sam@x.com' });
    const event = makeEvent({
      attendees: [SELF, { email: 'sam@x.com' }, { email: 'extra@x.com' }],
    });

    const result = matchEventToMember(event, [member], makeRules({ max_other_attendees: 2 }));
  ...
  it('exposes its helper functions as named exports', () => {
    expect(typeof passesAttendeeCap).toBe('function');
