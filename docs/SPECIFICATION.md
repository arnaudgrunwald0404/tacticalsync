...
**Channel allowlist**: the "Slack channels to include" picker lives in **Settings → Briefs & Schedule → Tools** (`CosPrepSchedulePanel.tsx`) and writes to `cos_prep_schedule.slack_channels`, which `slack-messages-sync`/`extract-inbox-action-items` now read directly on every invocation (cron included) — see [§13.11](#13-known-issues--drift-found-during-this-audit) for the fix history; this used to only take effect via manual "Sync now," not anymore.
