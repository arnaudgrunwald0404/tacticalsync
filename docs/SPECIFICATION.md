...
8. **No retry/backoff on any external integration** (Zoom/Slack/Gmail/Calendar) — flagged directly in `INTEGRATIONS.md` as a cross-cutting gap worth fixing centrally. **Not addressed** — larger cross-cutting feature work, out of scope for a targeted bug-fix pass.
...
15. **Unclear whether the legacy Socket Mode Slack bot (`slack-bot/`) is still deployed in production**, or is dev-only tooling that happens to share an action-ID naming convention with the live `agent-slack-action` endpoint. Confirm with whoever owns the Slack app configuration (§8.3) — this affects whether `slack-bot/` can be safely archived. **Not addressed** — needs confirmation from whoever owns the Slack app deployment, not a code fix.
...
- ~~Fix the Slack channel-allowlist column mismatch (§13.11)~~ — done, see PR #147. Still need to decide whether the legacy Socket Mode bot (§13.15) should be archived or kept as a second interactivity surface.
