...
| **Chief of Staff / Inbox** (§7) | Per-user | 1:1s, group meetings, `inbox_items`, delegations | Heavy — Gemini-driven extraction, nudging, delegation, relationship memory |
...
- **Other notable deps:** `reactflow` (Strategy Canvas), `recharts` (Insights), `xlsx` (import/export), `@dnd-kit` + `@hello-pangea/dnd` (two separate drag-and-drop libraries, used in different modules), `@stackone/hub` (third-party integrations), Google Gemini (`gemini-2.5-flash` for extraction/classification, `gemini-2.5-pro` for briefs/chat/delegation, via `GOOGLE_AI_API_KEY` and the shared `supabase/functions/_shared/gemini.ts` helper — all edge functions were ported off the Anthropic SDK/`ANTHROPIC_API_KEY` on 2026-08-28)
...
### Delete Recovery (capture + admin restore)
DO/SI/task deletes are hard deletes (`deleteDO`/`deleteInitiative`/`deleteTask` in `useRCDOMutations.ts`/`useTasks.ts`), including FK-cascaded rows (`rc_do_metrics`, child SIs, their `rc_tasks`) and the explicitly-cleaned-up `rc_links`/`rc_checkins` (no FK to their parent). `20260807234142_add_rc_deleted_items_audit_table.sql` adds `BEFORE DELETE` triggers on all six tables that snapshot each deleted row into `rc_deleted_items` (admin-only `SELECT` via RLS) before it's gone, grouped by a `batch_id` that defaults to the transaction id so a single cascading delete's rows land in one restorable batch. `20260807234201_add_restore_deleted_rc_batch_rpc.sql` adds `restore_deleted_rc_batch(p_batch_id text)`, an admin-only RPC that loops passes over a batch's un-restored rows, reinserting each in FK-safe table order (DOs → metrics → SIs → tasks → links → checkins) and retrying rows still blocked by an unrestored dependency — including `rc_strategic_initiatives.parent_si_id` self-references for sub-SIs — until a pass makes no further progress. Settings surfaces this via a new "Recently Deleted" nav item in the RCDO group (`showRecentlyDeleted` prop on `SettingsNavbar`, gated `isAdmin || isSuperAdmin`, same pattern as `showOrgTalkingPoints`/"Talking Points"): `RecentlyDeletedPanel.tsx` lists un-restored batches (what was deleted, by whom, when) with a "Restore" button that calls the RPC. `deleteDO`/`deleteInitiative` (`useRCDOMutations.ts`) run as single-transaction RPCs (`delete_rc_do`/`delete_rc_initiative`, `20260808000434_add_delete_rc_do_and_initiative_rpcs.sql`) rather than separate `rc_links`/`rc_checkins`/main-row Supabase JS calls, so every row one delete action touches lands in the same Postgres transaction and therefore the same `batch_id` — restoring a batch restores everything that action deleted. Each RPC is `SECURITY DEFINER` and replicates its table's actual DELETE RLS predicate internally (DO delete: admin/super-admin only; SI delete: owner or admin/super-admin) since `SECURITY DEFINER` bypasses RLS.
...
- Post-call AI analysis (`generate-meeting-suggestions` edge function): strips transcript markup, calls Gemini to extract action items, dedupes against existing items (Jaccard similarity), writes `dci_suggested_tasks`.
...
The Slack pipeline is OAuth-based edge functions: `exchange-slack-token` (per-user OAuth) → `slack-messages-sync` (pulls DMs + allowlisted channels, now on its own configurable hourly auto-sync schedule — see below) → `extract-inbox-action-items` (runs 4×/day via cron, delta-scans new Slack + Gmail messages, calls Gemini 2.5 Flash for structured extraction, writes `inbox_items(type='agent_question')` — **never auto-created as a real task**, requires explicit user approval). A legacy Socket Mode Bolt app (`slack-bot/index.js`) previously existed alongside this as dev-only tooling with an overlapping `/checkin` command; confirmed with the repo owner that no such app is deployed anywhere, so it was removed (§13.15).

**There are two parallel, independent Gmail/Slack-mining pipelines feeding the Inbox**, not one:
1. **`extract-inbox-action-items`** (Gemini 2.5 Flash) → `inbox_items(type='agent_question')`. This is where the sender-tier classification and suppression-learning system below lives. Gmail messages are fetched with `format=full` (not `metadata`): candidate links are pulled from the email body (`extractEmailBodyUrls()` in `inboxTriageUtils.ts` — hrefs from `text/html` parts + bare URLs from `text/plain`, unsubscribe/footer/social/asset links dropped, capped at 12) and offered to the extraction prompt, which may return the one link that is the action's direct target (e.g. the training an email asks you to complete). The returned URL is validated **verbatim** against that message's candidate list (a paraphrased/invented URL is discarded), then stored as `agent_payload.action_url`/`action_label`. The item detail sidebar (desktop `InboxAssistantPanel` and mobile `InboxItemDrawer`) renders a **Source** section (`SourceLinks.tsx`) for email/Slack-sourced items: a deep link to the original email (`agent_payload.gmail_url`, falling back to `#all/<source_ref.id>` for items that predate `gmail_url`) or Slack message (`slack_url`), plus the extracted action link when present.
...
**Already-answered suggestions auto-archive on each scan** (both sources): `reconcileAnsweredSlackItems()` archives an open Slack `agent_question` item once `cos_slack_messages` shows the user posted in that channel/DM after the flagged message (§13.26, PR #255), and its Gmail counterpart `reconcileAnsweredGmailItems()` archives an open Gmail item once the source thread (looked up via the Gmail API — threadId parsed from `agent_payload.gmail_url`, falling back to a `messages.get`) contains a `SENT` message dated after the flagged one (`hasUserReplyAfter()` in `inboxTriageUtils.ts`). Both run on every `extract-inbox-action-items` invocation whenever the source is connected — *not* gated on the triage toggles, so stale items clear even on runs with no fresh scan. Items are set to `status='archived'` directly, deliberately **without** an `inbox_dismissal_log` entry: dismissal entries train sender suppression, and "I handled this" is the opposite signal from "stop showing me this sender". Reconciliation runs on the 4×/day cron, the settings panels' "Scan inbox" buttons, **and the suggestions panel's refresh button** — `useMeetingSuggestions.refresh` (used by `InboxSuggestionsPanel`) invokes `extract-inbox-action-items` alongside its meeting/Slack/Gmail re-syncs and then reloads the `inbox_items` query behind the panel via `onAfterRefresh` (user-reported: an already-replied-to Gmail suggestion survived refresh because refresh previously only ran the three other functions — same class as §13.19 root cause 3 — leaving a worst-case ~6-hour window until the next cron cleared it).

**The suggestions panel's re-scan button is incremental with cost guardrails.** The `RefreshCw` button (`useMeetingSuggestions.refresh`, shared by `InboxSuggestionsPanel` and `SuggestedFromMeetingsPanel`) invokes all four pipelines — `generate-meeting-suggestions`, `slack-inbox-sync`, `gmail-inbox-sync`, and `extract-inbox-action-items` — but each is bounded server-side so a click never re-runs a full scan:
- **Per-user scan cursors** (`cos_action_item_scan_state`; sources `slack_suggestions`/`gmail_suggestions` for pipeline 2, alongside `extract-inbox-action-items`' pre-existing `slack`/`gmail` rows): each sync fetches only content newer than its last completed scan, with a 24h overlap so threads whose extraction failed (excluded from `suggestion_source_processed`) are retried. Meeting scanning was already incremental via `cos_zoom_transcripts.suggestions_extracted_at`.
- **10-minute cooldown** (`_shared/scanCursor.ts`): if a source's cursor is under 10 minutes old, that function skips its fetch + LLM extraction entirely — rapid re-clicks cost DB queries, not model calls.
- **Validation + dismissal sweeps run on every click, cooldown or not, and are LLM-free**: `resolveAnsweredSlackSuggestions()`/`resolveAnsweredEmailSuggestions()` set pending `dci_suggested_tasks` to the new `status='resolved'` when the user has since replied in the source Slack channel / Gmail thread (a distinct status on purpose — the `stamp_suggestion_outcome` trigger only stamps `outcome_at` for accepted/dismissed, so auto-resolved rows never pollute the few-shot learning history); `applySuppressionToOpenItems()` in `extract-inbox-action-items` retroactively archives open `agent_question` items matching the *current* suppression rules (learned senders/domains/intents plus `isAutomatedSender()`), which were previously only checked at item-creation time. Suppression inference also now runs whenever a source is connected+enabled (not only after a fresh scan), so rules learned from recent dismissals apply on the same click. After the refresh, `Inbox.tsx` reloads its item lists (`onReloadAgentItems`) so server-archived agent items actually leave the panel, and the button's tooltip surfaces the newest `cos_action_item_scan_state.last_scanned_at` ("last scanned N minutes ago") so the cooldown is legible instead of silent. Migration: `20260901120000_rescan_incremental_guardrails.sql`.
...
| 12 | **Nudge ceiling** (meeting action items only) | An action has been nudged `nudge_max_count` times (default 5) with no resolution | Action is silently "parked" (logged `nudge_capped`) — stops appearing in future digests but remains resolvable via its normal overflow actions. **Due-date inbox nudges no longer use a ceiling**: each item is nudged at most **once per deadline** (the `agent_nudge` inbox row + Slack DM fire a single time; a repeat renders identically and is pure noise). The only way an item is nudged again is if its `priority_due_at` is moved afterwards — e.g. the Slack "Snooze 2 days" action — which counts as a new deadline. Legacy `inbox_due_nudge_capped` markers from the retired repeat-then-cap scheme still permanently silence their items |
...
Every Gemini call made by an edge function (19 call sites across 16 functions as of Sep 2026) logs one row to `ai_usage_log` (function name, model, input/output token counts, optional user, duration). Logging lives inside the shared provider wrapper — `geminiGenerateText`/`geminiChat` in `supabase/functions/_shared/gemini.ts` accept a `log: { functionName, userId? }` option and call `_shared/aiUsage.ts` (a fire-and-forget service-role insert that never throws) — so any future call site gets cost tracking by passing one option rather than writing its own insert. A daily rollup view `ai_usage_daily` (`security_invoker = true`, so the base table's admin-only RLS applies) feeds the **Settings → AI Usage** panel (`src/components/settings/AiUsagePanel.tsx`, gated `is_admin`/`is_super_admin`): a stacked daily cost chart by function, summary tiles, and a per-function table with avg-cost-per-call as the optimization signal. Dollar cost is computed at display time from `src/lib/aiPricing.ts` (per-MTok rates keyed by normalized model id — Gemini models for current rows, Anthropic models for rows logged before the Aug 2026 provider port) so price updates never require rewriting logged history. The older `prep_generation_log` (migration `20260607100000`) predates this and still drives prep rate-limiting. Migration: `20260901090000_ai_usage_log.sql`.
...
5. ~~**`rc_tasks` RLS was effectively broken for most cycles**~~ **RESOLVED in `20260807025211_fix_rc_tasks_company_wide_access.sql`.** A prior pass through this list marked this "VERIFIED RESOLVED (no code change needed)" after confirming `20260521000000_fix_rc_tasks_rls_admin_bypass.sql` OR's owner/creator/admin bypasses into all four policies — but that audit never actually exercised the INSERT/SELECT policies as a non-owner, non-admin user, which is exactly the case those bypasses don't cover. `useRCDOPermissions.ts`'s `canCreateTask()` returns `true` for every authenticated user (comment: "This will be enforced by RLS policies"), but the INSERT policy still required SI ownership, DO ownership, or a `team_members` row reached via `rc_cycles.team_id` — dead ever since `20251112100000_make_rcdo_company_wide.sql` set every cycle's `team_id` to `NULL`. Reproduced live (42501 `new row violates row-level security policy`) via the Add Task dialog as a non-owner, non-admin user. Fixed by opening `rc_tasks` SELECT/INSERT to any authenticated user, matching `rc_strategic_initiatives`'s company-wide SELECT and `rc_links`'s open INSERT policy from the same company-wide migration. UPDATE/DELETE were left unchanged — they already grant exactly what `canEditTask`/`canDeleteTask` expect (task owner, SI owner, or admin).
...
26. ~~**`extract-inbox-action-items` surfaced no-reply/automated senders, calendar invites, and Slack questions the user had already answered directly in Slack.**~~ **RESOLVED in [PR #255](https://github.com/arnaudgrunwald0404/tacticalsync/pull/255).** User-reported: the "Suggested from your email, Slack" panel kept showing items the user had explicitly asked (in a prior chat session) to be filtered — the feedback had only ever been saved to that chat session's own memory, which this cron-driven edge function has no access to; the actual extraction logic was untouched. Root causes: (1) the Claude Haiku extraction prompt explicitly told the model automated/no-reply emails "still qualify" if they looked actionable, and there was no static sender-pattern filter — only a *learned* suppression requiring ≥5 prior dismissals of the same sender; (2) no filter existed at all for calendar invite/RSVP emails; (3) once a Slack `agent_question` item was created, nothing ever re-checked it — the existing dedup only prevented inserting a second copy of the same item, it never closed one out even after the user replied to the underlying Slack message directly. Fixed by adding `isAutomatedSender()`/`isCalendarInvite()` static filters (`_shared/inboxTriageUtils.ts`) applied before Gmail messages reach the model, and a new `reconcileAnsweredSlackItems()` step that auto-archives an open Slack `agent_question` item once `cos_slack_messages` shows the user posted in that same channel/DM after the flagged message. **Follow-up:** the same gap existed for Gmail items (user-reported again with a suggestion whose email had already been answered) — `reconcileAnsweredGmailItems()` now auto-archives an open Gmail `agent_question` item once the source thread contains a user-`SENT` message dated after the flagged one, checked via the Gmail API on every run where Gmail is connected; see §7.7. **Second follow-up:** `isCalendarInvite()`'s subject-prefix list missed the propose-new-time family — "Proposed new time:" (Google Calendar) / "New Time Proposed:" (Outlook), plus "Tentatively Accepted:" and "Meeting Forward Notification:" — which are sent from the counterparty's own address, so sender-based filtering can't catch them (user-reported: a "Proposed new time: Kristin / Arnaud @ …" email surfaced as a suggestion; the same subject shape had slipped through at least twice before).
...
| RCDO | `rc_cycles`, `rc_rallying_cries`, `rc_defining_objectives`, `rc_do_metrics`, `rc_strategic_initiatives`, `rc_tasks`, `rc_checkins`, `rc_links`, `rc_canvas_states`, `rc_deleted_items` |
